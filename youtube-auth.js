...
const SCOPES = [
  ...
  'https://www.googleapis.com/auth/youtube.force-ssl', // commentThreads.insert (course-link comment)
...
function openBrowser(url) {
  ...
  try {
    // Detach so the parent process can exit independently.
    let proc;
    if (platform === 'darwin') {
      proc = spawn('open', [url], { stdio: 'ignore', detached: true });
    } else if (platform === 'win32') {
      // cmd /c start "" "<url>" — the empty title arg is required by start.
