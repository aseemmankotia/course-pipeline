...
// Keys are read at call time so dotenv can be loaded by the caller first
function _keys() {
  return {
...
async function _callAnthropic({ prompt, systemPrompt, maxTokens, action }) {
  ...
  const body = {
    model:      process.env.COURSE_GEN_MODEL || 'claude-sonnet-5',
  ...
  if (!resp.ok) {
    ...
    if (_isBalanceError(resp.status, msg)) {
      ...
      return null; // signal: fall through to Gemini
    }
...
async function _callGemini({ prompt, systemPrompt, maxTokens, action }) {
  ...
  if (!apiKey) return null; // signal: no key

  const model     = 'gemini-2.5-flash';
  const url       = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`;
  const outTokens = Math.min((maxTokens || 4000) * 2, 8192);

  // Combine system + user into a single user turn (Gemini REST doesn't take a
  // separate system field in the same way; prepending is reliable)
 ...
 * @param {string}  [opts.action='unknown']   label for log lines
...
async function callAI({ prompt, systemPrompt = '', maxTokens = 4000, action = 'unknown' }) {
  ...
  if (anthropic) {
    try {
      ...
      if (text !== null) return text; // success or balance-error fall-through (null)
    } catch (err) {
      // Non-balance Anthropic error — rethrow immediately
      throw err;
    }
  }
