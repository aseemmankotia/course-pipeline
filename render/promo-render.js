...
async function screenshotSlides(slides, course) {
  ...
  const browser    = await puppeteer.launch({ headless: true, protocolTimeout: 240000, args: ['--disable-gpu', '--use-angle=swiftshader', '--disable-accelerated-2d-canvas', '--hide-scrollbars', '--force-color-profile=srgb', '--disable-dev-shm-usage'] });
  const slidePaths = [];
